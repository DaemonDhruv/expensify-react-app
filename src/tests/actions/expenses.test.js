...
const userId = 'thisismytestuserId';
...
beforeEach(done => {
    ...
    database.ref(`users/${userId}/expenses`).set(expenseData).then(() => done());
});
...
test('should remove expenses from firebase', (done) => {
    ...
    store.dispatch(startRemoveExpense(expenses[0].id)).then(() => {
        ...
        return database.ref(`users/${userId}/expenses/${actions[0].id}`).once('value');
     }).then((snapshot) => {
         expect(snapshot.val()).toBeFalsy(); // if getting null back
         done();
     });
});
